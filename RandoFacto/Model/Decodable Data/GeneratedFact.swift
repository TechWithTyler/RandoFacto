...
struct GeneratedFact: Codable {
    ...
    // When decoding data, your Codable type must not have any more properties than what the encoded data has. For example, if you're decoding JSON data that has name and age properties, the Codable type can't also have a birthYear property or else it can't decode properly. Your Codable type can, however, have fewer properties than the encoded data. In the case of GeneratedFact, it contains fewer properties than the original encoded JSON data. because that's the only property this app is using.
